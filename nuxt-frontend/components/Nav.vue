<template>
  <div class="navbar flex text-white relative">
    <div class="nav-item-center flex space-x-6 sm:space-x-20 p-5 mx-auto">0
      <nuxt-link :to="localePath('/')">{{ $t('Home') }}</nuxt-link>
      <nuxt-link :to="localePath('all')">{{ $t('All') }}</nuxt-link>
      <nuxt-link :to="localePath('men')">{{ $t('Men') }}</nuxt-link>
      <nuxt-link :to="localePath('women')">{{ $t('Women') }}</nuxt-link>
    </div>
